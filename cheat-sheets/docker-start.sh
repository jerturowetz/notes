...
# This is a copy of the file created by docker toolbox
# It's purpose is to start the virtualbox machine upon which docker runs (usually "default")
# This script must be run every time the system is restarted in order to have a machine on which to run docker
# Obvs, it's not necessary for systems capable of running docker without docker toolkit
